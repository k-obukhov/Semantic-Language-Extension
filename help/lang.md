...
* Скалярные
  * Встроенные типы:
    * ```integer``` — целые 32-битные знаковые числа
    * ```float``` — вещественные 32-битные знаковые числа
    * ```char``` — символ (1 байт)
...
```
variable integer MyIntVar; // variable только для встроенных скалярных типов и типов, определенных пользователем
...
public readonly string S := "ffff"; // readonly значит, что поле можно изменять только в данном модуле, во всех остальных его можно только читать!
```
## Дополнительно
